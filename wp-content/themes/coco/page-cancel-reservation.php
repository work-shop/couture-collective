...
if ( isset( $_POST['referring-page']) && isset($_POST['user-id']) && isset($_POST['booking-id']) ) {
	if ( get_current_user_id() == ( $id = $_POST['user-id'] ) ) {
		...
		if ( $val ) {
			...
			// why isn't the response switch appropriate here?
			...
			if ( $or_succ && $bk_succ ) {
				...
				wc_add_notice( '(1) we couldn\'t cancel your booking!','notice' );
				wp_redirect( $_POST['referring-page'] );
				exit;
			}
		} else {
			// internal cancellation error.
			wc_add_notice( '(2) we couldn\'t cancel your booking!','notice' );
			wp_redirect( $_POST['referring-page'] );
			exit;
		}
	} else {
