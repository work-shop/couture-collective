...
class WC_Subscriptions_Renewal_Order {
	...
	public static function get_renewal_orders( $order_id, $output = 'ID' ) {
		...
		$renewal_order_ids = $wpdb->get_col( $wpdb->prepare( "SELECT ID FROM {$wpdb->posts} WHERE post_parent = %d AND post_type = 'shop_order' ORDER BY {$wpdb->posts}.post_date ASC", $order_id ) );
