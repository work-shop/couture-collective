...
class WC_PayPal_Standard_Subscriptions {
	...
	public static function process_paypal_ipn_request( $transaction_details ) {
		...
		if ( false !== strpos( $transaction_details['invoice'], '-wcsfrp-' ) && in_array( $transaction_details['txn_type'], array( 'subscr_signup', 'subscr_payment' ) ) ) {
			...
			// Now set the correct old profile ID on the order (for the first IPN message that comes through)
			$existing_profile_id = get_post_meta( $order->id, 'PayPal Subscriber ID', true );

			if ( $existing_profile_id !== $transaction_details['subscr_id'] ) {
		...
		if ( 'paypal' != $order->recurring_payment_method && false === $is_renewal_sign_up_after_failure ) {
			if ( self::$debug ) {
		...
		switch( $transaction_details['txn_type'] ) {
			case 'subscr_signup':
				...
				$default_invoice_string = self::$invoice_prefix . ltrim( $order->get_order_number(), '#' );
