							...
							if ('false' == WCSubscriptions.isWCPre22) {
								...
								var interval = setInterval(function() {
									if ($('#woocommerce-order-items').data('blockUI.isBlocked') != 1){
										...
										$('#order_line_items tr.item').each(function(row_idx){
											if (row_idx !== idx){
												return true;
											}
											...
											$(this).find('[value="_recurring_line_subtotal_tax"]').next('[name^="meta_value"]').val( result.recurring_line_subtotal_tax );
											$(this).find('[value="_recurring_line_tax"]').next('[name^="meta_value"]').val( result.recurring_line_tax );
